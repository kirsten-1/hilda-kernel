"""
GeGLU Benchmark: Ours vs Liger vs PyTorch

GeGLU(a, b) = GELU(a) * b  (tanh approximation)
...
from kernels.geglu import hilda_geglu

# 尝试导入 Liger
try:
    from liger_kernel.ops.geglu import LigerGELUMulFunction
    HAS_LIGER = True
    print("Liger GeGLU found.")
except ImportError:
    HAS_LIGER = False
    print("Liger GeGLU not found.")
...
def quick_test():
    ...
    configs = [
        (512,  11008,  torch.float16,  "D=11008, fp16"),
        (512,  14336,  torch.float16,  "D=14336, fp16"),
        (512,  14336,  torch.bfloat16, "D=14336, bf16"),
        (512,  28672,  torch.float16,  "D=28672, fp16"),
        (512,   4096,  torch.float32,  "D=4096,  fp32"),
    ]
    ...
    for N, D, dtype, name in configs:
        ...
        ref  = pytorch_geglu(a, b)
        ours = hilda_geglu(a, b)
...
def backward_test():
    ...
    configs = [
        (512, 11008, torch.float16,  "D=11008, fp16"),
        (512, 14336, torch.float16,  "D=14336, fp16"),
        (512, 14336, torch.float32,  "D=14336, fp32"),
    ]
...
def bench_fwd_bwd(N, D, dtype, device='cuda'):
    ...
    if HAS_LIGER:
        def liger_fn():
            a = torch.randn(N, D, device=device, dtype=dtype, requires_grad=True)
            b = torch.randn(N, D, device=device, dtype=dtype, requires_grad=True)
            out = LigerGELUMulFunction.apply(a, b)
            out.sum().backward()
        try:
...
def main():
    print("=" * 70)
    print("GeGLU Benchmark: Ours vs Liger vs PyTorch")
    print("=" * 70)
