"""
AttnRes (Attention Residuals) - Kimi/Moonshot AI

替代标准残差连接 h_l = h_{l-1} + f_l(RMSNorm(h_{l-1}))，
用 softmax attention over depth 动态加权：
...
解决 PreNorm dilution：深层 layer 贡献被稀释的问题。
论文: arxiv.org/abs/2603.15031

Triton 优化：
1. 单 kernel 融合 RMSNorm + dot + softmax + weighted sum
2. 每个 program 处理一个 (batch, token) 位置
3. N 很小 (≤16 blocks)，scores 全在寄存器
"""
...
class HildaAttnResFunction(torch.autograd.Function):
    @staticmethod
...
def hilda_attn_res(blocks, w_query, w_norm, eps=1e-6):
    """
    Functional interface for AttnRes.
    blocks: [N, B, T, D] stacked or list of [B, T, D]
    w_query: [D] learned pseudo-query
    w_norm: [D] RMSNorm weight
    Returns: [B, T, D]
    """
    if isinstance(blocks, (list, tuple)):
        ...
        V = blocks
    return HildaAttnResFunction.apply(V, w_query, w_norm, eps)
...
class AttnRes(torch.nn.Module):
    """
    Attention Residuals module (Kimi/Moonshot AI).
    forward(blocks) -> h, where blocks is [N, B, T, D] or list of [B, T, D]
    """
    def __init__(self, hidden_dim, eps=1e-6):
        super().__init__()
        self.eps = eps
        self.w_query = torch.nn.Parameter(torch.randn(hidden_dim) * 0.02)
        self.w_norm  = torch.nn.Parameter(torch.ones(hidden_dim))

    def forward(self, blocks):
        return hilda_attn_res(blocks, self.w_query, self.w_norm, self.eps)
